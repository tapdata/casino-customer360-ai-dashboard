...
/**
 * Validate and optionally upload TapData connection/task exports.
 *
 * TapData export/import endpoints are edition/version specific, so this tool
 * deliberately has no guessed endpoint defaults.  `manual` mode is safe for
 * preparing a hand-off manifest; `api` mode is opt-in and requires the exact
...
function createManifest(connectionPath, taskPath) {
  ...
  return {
    ...
    requiresSourceUri: !env.TAPDATA_SOURCE_MONGODB_URI && !asBool(env.TAPDATA_IMPORT_ALLOW_MISSING_URI),
    requiresEndpointConfig: !env.TAPDATA_CONNECTION_IMPORT_PATH || !env.TAPDATA_TASK_IMPORT_PATH,
...
function authHeaders() {
  const headers = {};
  if (env.TAPDATA_IMPORT_TOKEN) headers.Authorization = `Bearer ${env.TAPDATA_IMPORT_TOKEN}`;
  return headers;
}

async function request(path, { method = "GET", form, body } = {}) {
  const base = env.TAPDATA_API_BASE_URL;
  if (!base) throw new Error("TAPDATA_API_BASE_URL is required in api mode");
  ...
  try {
    ...
    return { status: response.status, contentType, summary: parsed && typeof parsed === "object" ? Object.keys(parsed).slice(0, 20) : [] };
  } finally {
    clearTimeout(timer);
  }
}
...
async function runApiImport(connectionPath, taskPath) {
  const connectionPathApi = env.TAPDATA_CONNECTION_IMPORT_PATH;
  const taskPathApi = env.TAPDATA_TASK_IMPORT_PATH;
  if (!connectionPathApi || !taskPathApi) {
    throw new Error("api mode requires TAPDATA_CONNECTION_IMPORT_PATH and TAPDATA_TASK_IMPORT_PATH; no endpoint is guessed");
  }
  if (!env.TAPDATA_SOURCE_MONGODB_URI && !asBool(env.TAPDATA_IMPORT_ALLOW_MISSING_URI)) {
    throw new Error("connection export omits the MongoDB URI; set TAPDATA_SOURCE_MONGODB_URI or explicitly allow missing URI");
  }
  if (env.TAPDATA_SOURCE_MONGODB_URI && !env.TAPDATA_IMPORT_URI_FIELD) {
  ...
  const taskResult = await uploadExport(
    taskPathApi,
    taskPath,
    env.TAPDATA_IMPORT_TASK_FILE_FIELD || "taskFile",
    { ...commonFields, ...taskFields },
  );
...
async function main() {
  ...
  const connectionPath = resolveArtifact(
  ...
  const manifest = createManifest(connectionPath, taskPath);
  ...
  if (importMode !== "api") {
    log("manual mode: import the connection first, then the task in TapData UI/API; the XLSX export does not contain the source MongoDB URI");
    log("no network calls were made");
    return;
  }

  await runApiImport(connectionPath, taskPath);
}
